(() => {
 ...
 temaRogzitettbol=function(kiemelt,jelolt,regio){
  ...
  const matches=all.filter(h=>canonical(h.link)!==canonical(fo.link)&&Math.abs(Date.now()-new Date(h.datum).getTime())<=7*24*3600000&&(nhSameEvent(fo,h)||keywordMatch(h)));
 ...
 if (osszesHir && osszesHir.length){
  rajzol();
  refreshRomanPress();
  window.__NH_EDITORIAL_READY__ = true; window.__NH_REVEAL__?.();
 }
})();
